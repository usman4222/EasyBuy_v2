...
const AllUsersTable = () => {
  ...
  const { success, isDeleted } = useSelector((state) => state.userProfile); 

  useEffect(() => {
    dispatch(getAllUsers(currentPage, rowsPerPage)); // Dispatch action to fetch users with pagination
  ...
  const confirmDelete = () => {
    dispatch(deleteUser(userIdToDelete)); // Dispatch delete action with userIdToDelete
  ...
  const handleEdit = (userId, currentRole) => {
    ...
    setNewRole(currentRole); // Set the current role in the input
    setIsModalOpen(true); // Open the modal
  };

  const handleRoleUpdate = () => {
    // Prepare userData with the new role
    const userData = { role: newRole };
    // Dispatch the updateUser action to update the role
            ...
            users.map((user, index) => (
              <tr key={user._id} className="border-b">
                ...
                <td className="p-4 flex space-x-2">
                  {/* Edit Icon */}
                  <button
                    onClick={() => handleEdit(user._id, user.role)} // Pass user id and current role
          ...
          <div className="bg-white p-6 rounded-lg w-96">
            ...
            <div className="flex justify-between">
              ...
              <button
                ...
                onClick={cancelDelete} // Close modal without deleting
              >
                Cancel
              </button>
            </div>
