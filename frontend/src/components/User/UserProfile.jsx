...
const UserProfile = () => {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      // Update the profile image in the local state (or Redux if using)
      setUpdatedUser((prev) => ({
        ...prev,
        profileImage: data.secure_url, // Correctly update profileImage
      }));

      // Optional: If you use Redux or another global state management, dispatch the update.
      // dispatch(updateUser({ ...currentUser, profileImage: data.secure_url }));

      alert("Image uploaded successfully!");
      console.log("Uploaded image URL:", data.secure_url); // Check that this is the correct URL
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto p-6 bg-[#f3f3f3] rounded-xl shadow-xl">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center md:justify-between bg-white p-8 rounded-2xl shadow-lg transform hover:scale-105 transition duration-300 ease-in-out">
          ...
          <div className="mb-6 md:mb-0 md:w-48 w-36 h-36">
            <img
              ...
              alt="Profile"
              className="w-full h-full object-cover rounded-full border-4 border-indigo-600 shadow-xl"
            />
          </div>
            ...
            <div className="md:ml-8 text-center md:text-left">
              <form onSubmit={handleSubmit}>
                ...
                <div className="mt-4 text-center">
                  <button
                    type="submit"
                    className="bg-indigo-600 text-white py-2 px-6 rounded-full shadow-lg hover:bg-indigo-700 transition duration-300 ease-in-out"
                  >
                    Save Changes
                  </button>
                </div>
        ...
        <div className="mt-10 text-center">
          <button
            ...
            className="bg-indigo-600 text-white py-3 px-6 rounded-full shadow-lg hover:bg-indigo-700 transition duration-300 ease-in-out transform hover:scale-105 mr-4"
          >
            {isEditing ? "Cancel" : "Edit Profile"}
          </button>
          <button onClick={handleLogout} className="bg-primaryRed text-white py-3 px-6 rounded-full shadow-lg hover:bg-red-700 transition duration-300 ease-in-out transform hover:scale-105">
            Logout
          </button>
        </div>
      </div>
